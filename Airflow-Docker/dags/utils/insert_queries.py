...
def insert_query_customer(**kwargs):
    ...
    insertQuery = "DELETE FROM customer;\n"
    dataframe =cargar_datos(kwargs['csv_path'])
    for index, row in dataframe.iterrows():
...
def insert_query_employee(**kwargs):
    ...
    insertQuery = "DELETE FROM employee;\n"
    dataframe =cargar_datos(kwargs['csv_path'])
    for index, row in dataframe.iterrows():
...
def insert_query_stock(**kwargs):
    # To Do
    insert = f"INSERT INTO stockitem (Stock_Item_Key,WWI_Stock_Item_ID,Color,Selling_Package,Buying_Package,Brand,Size_val,Lead_Time_Days,Quantity_Per_Outer,Is_Chiller_Stock,Tax_Rate,Unit_Price,Recommended_Retail_Price,Typical_Weight_Per_Unit) VALUES "
    insertQuery = "DELETE FROM stockitem;\n"
    dataframe =cargar_datos(kwargs['csv_path'])
    for index, row in dataframe.iterrows():
        insertQuery += insert + f"({row.Stock_Item_Key},\'{row.WWI_Stock_Item_ID}\',\'{row.Color}\',\'{row.Selling_Package}\',\'{row.Buying_Package}\',\'{row.Brand}\',\'{row.Size_val}\',\'{row.Lead_Time_Days}\',\'{row.Quantity_Per_Outer}\',\'{row.Is_Chiller_Stock}\',\'{row.Tax_Rate}\',\'{row.Unit_Price}\',\'{row.Recommended_Retail_Price}\',{row.Typical_Weight_Per_Unit});\n"
    return insertQuery
...
def insert_query_fact_order(**kwargs):
    ...
    insertQuery = "DELETE FROM fact_order;\n"
    dataframe =cargar_datos(kwargs['csv_path'])
    for index, row in dataframe.iterrows():
